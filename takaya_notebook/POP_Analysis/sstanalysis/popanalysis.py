...
class POPFile(object):
    ...
    def __init__(self, fname, ah=-3e17, is3d=False):
        ...
        self.mask = self.nc.variables['REGION_MASK'][:] != 2
    ...
    def power_spectrum_2d(self, T, lonrange=(154.9,171.7), latrange=(30,45.4)):
        """Calculate a two-dimensional power spectrum of netcdf variable 'varname'
           in the box defined by lonrange and latrange.
        """
    
        tlon = np.roll(self.nc.variables['TLONG'][:],-1000)
        tlat = np.roll(self.nc.variables['TLAT'][:],-1000)
        ...
        T = np.roll(T, -1000)[..., jmin:jmax, imin:imax]
        ...
        Pacific_mask = np.roll(self.mask,-1000)[jmin:jmax, imin:imax]
        land_fraction = Pacific_mask.sum().astype('f8') / (Ny*Nx)
        if land_fraction==0.:
            # no problem
            pass
        ...
        for n in range(Nt):
            Ti = np.ma.masked_array(T[n], Pacific_mask)
